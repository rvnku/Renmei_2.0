...
class Node(Generic[ClientT]):
    ...
    async def close(self) -> None:
        ...
        .. versionadded:: 2.6
        """
        await self._disconnect_players_for_shutdown("Node is closing")
    ...
    async def _disconnect_players_for_shutdown(self, reason: str) -> None:
        """Disconnect all players while the node can still clean up voice state."""
        if not self._players:
            return

        players = list(self._players.values())
        _log.warning(
            "%s, disconnecting %d player(s).",
            reason,
            len(players),
            extra={"label": self._label},
        )
        results = await gather(
            *(player.disconnect(force=True) for player in players),
        ...
        for player, result in zip(players, results):
            if isinstance(result, Exception):
                _log.error(
                    "Failed to disconnect player %s during node shutdown.",
                    player.guild.id,
                    extra={"label": self._label, "guild": player.guild.id},
                    exc_info=result,
                )

    async def _destroy_players_on_unavailable(self) -> None:
        """Disconnect all players when the node becomes unavailable."""
        await self._disconnect_players_for_shutdown("Node became unavailable")
    ...
    async def _ws_listener(self) -> None:
        ...
        while True:
            ...
            if _type in (
                ...
                await self._destroy_players_on_unavailable()

                if _type is aiohttp.WSMsgType.ERROR:
                    _log.error(
                        "Websocket listener encountered an error: %r",
                        msg.data,
                        extra={"label": self._label},
                    )
                ...
                task.add_done_callback(remove_task)
                return
            elif _type is not aiohttp.WSMsgType.TEXT:
                _log.debug(
                    "Skipping non-text websocket message with type %s.",
                    _type,
                    extra={"label": self._label},
                )
                continue
